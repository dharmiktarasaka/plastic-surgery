...
export default function ReconstructivePage() {
  const category = procedureCategories.find(c => c.id === 'reconstructive') || procedureCategories[5];

  return (
    <SubpageLayout>
      <Helmet>
        <title>Reconstructive Surgery | TARASAKA Centre</title>
        <meta name="description" content="Microsurgical reconstructive surgery restoring form, symmetry, and function after trauma or oncological resection." />
      </Helmet>
            ...
            <div key={proc.id} className="space-y-6">
              ...
              <div className="pt-4">
                <Link to="/book-consultation" className="px-8 py-3.5 rounded-full bg-[#3A243F] text-[#FBEFEF] text-xs font-semibold uppercase tracking-wider inline-block">
                  Schedule Reconstructive Evaluation
                </Link>
              </div>
            </div>
          ))}
